  ...
  businessesConfig.map((business) => {
    if(town.businesses[business.name]) {
      ...
      Object.entries(business.produces).map((product) => {
        ...
        production[product[0]] += numberOfBusinesses * product[1] * bonusProductionMulti
      })
...
export function calculateProductionInHansa(towns, goodsConfig, businessesConfig) {
  let perProduct = {}
  goodsConfig.map((good) => {
  ...
  towns.map((town) => {
    ...
    Object.entries(perProduct).map((productEntry) => {
      perProduct[productEntry[0]].production += inTown.production[productEntry[0]] || 0
      perProduct[productEntry[0]].required += inTown.requirements[productEntry[0]] || 0
      perProduct[productEntry[0]].numberOfBusinesses += inTown.businessesPerProduct[productEntry[0]] || 0
      perProduct[productEntry[0]].citizenNeeds += inTown.citizenNeeds[productEntry[0]] || 0
    })
  })
  return perProduct
}
