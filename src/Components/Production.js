...
class Production extends Component {
  render() {
    let perProduct = calculateProductionInHansa(
      this.props.towns,
      goodsConfig,
      businessesConfig
    );
    let goodsTd = Object.entries(goodsConfig).map(entry => {
      ...
      let totalNeed =
        perProduct[goodsName].production -
        perProduct[goodsName].required -
        perProduct[goodsName].citizenNeeds;
      let businessesNeeded = Object.entries(businessesConfig).map(entry => {
        if (entry[1].produces[goodsName]) {
          ...
          return (
            <span>
              {entry[1].name}:{' '}
              {Math.ceil(totalNeed / entry[1].produces[goodsName])} <br />
            </span>
          );
        }
        return '';
      ...
      return (
        <tr>
          ...
          <td>
            {Math.round(
              perProduct[goodsName].production - perProduct[goodsName].required
            )}
          </td>
